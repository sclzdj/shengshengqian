...
if (!function_exists('api_get_file_path')) {
    /**
     * API接口获取附件路径
     * @param int $id 附件id
     * @author 蔡伟明 <460932465@qq.com>
     * @return string
     */
    function api_get_file_path($id = 0)
    {
        $path = model('admin/attachment')->getFilePath($id);
        if (!$path) {
            return getRequestHost().config('public_static_path').'admin/img/none.png';
        }
        if(strpos($path, 'http') !== false)
        {
            return $path;
