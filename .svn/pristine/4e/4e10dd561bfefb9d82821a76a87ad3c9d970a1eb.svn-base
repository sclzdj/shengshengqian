...
class App
{
    ...
    public static $pluginsNameSpace = 'plugins';
    ...
    public static function module($result, $config, $convert = null)
    {
        ...
        if ($config['app_multi_module']) {
            ...
            if ($bind) {
                ...
                //todo 如果没有在拒绝访问的模块中， 并且在 application找到目录 或则 在插件plugins目录中找到
            } elseif (!in_array($module, $config['deny_module_list']) && (is_dir(APP_PATH . $module) || is_dir(ROOT_PATH.DS.'plugins'.DS.$module) )) {
                $available = true;
                if(is_dir(ROOT_PATH.DS.'plugins'.DS.$module))
                {
                    $request->setIs_plugins_module(true);
                    $isPlugins = true;//插件模式
                }
            }
        ...
        // 当前模块路径
        App::$modulePath = $isPlugins ? ROOT_PATH.DS.'plugins'.DS.$module.DS : APP_PATH . ($module ? $module . DS : '');
