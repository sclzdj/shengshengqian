...
class Apibase extends Controller
{
 ...
 /**
    ...
    public function getMid()
    {
        if ($this->getUserId() > 0) {
            if ($this->_mid > 0) {
                ...
                // 数据库查询
                $user = Users::get($this->getUserId());
                if (! $user) {
                    return 0;
                } else {
                    $this->setMid($user['mid']);
                    return $user['mid'];
                }
            }
        } else {
    ...
    {
        $appid = intval($this->_appid) ? intval($this->_appid) : $this->getRequest('appid',0) ;
        if($appid <= 0)
        {
            $this->response([], 201, 'need appid');
        }else{
            return $appid;
        }
    }
    ...
    protected function _initialize()
    {
        ...
        if($_POST)
        {
            $log = [
                ...
                'host'=>getRequestHost(),
                'user_agent'=>Request::instance()->header('user_agent',''),
                'created'=>time(),
                'ip'=>get_client_ip(),
                'path'=>sprintf("%s/%s/%s",$this->_module, $this->_controller, $this->_action)
            ];
            Db::name("api_log")->insert($log);
        }
