...
/**
 * Exchangeordertrue 后台模块
 */
class Exchangeordertrue extends Admin
{
	//银行管理
	public function index()
	{
		...
		// 读取银行数据
		$data_list = Db::name('exchange_order a')->join('exchange_product b','a.pid=b.id','LEFT')->join('users c','a.user_id=c.id','LEFT')->field('a.*,b.title,c.username')->where($map)->where('a.p_type','1')->order($order)->paginate();
		// 分页数据
		$page = $data_list->render();
		// 使用ZBuilder快速创建数据表格
        return ZBuilder::make('table')
        	->setPageTitle('兑换实物商品列表') // 设置页面标题
