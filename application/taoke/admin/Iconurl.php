...
class Iconurl extends Admin
{
	...
	public function add()
	{
		if (Request::instance()->isPost()) {
			...
			if($icon_url_id>0){
				return $this->success('添加首页模块成功');
	        } else {
	            return $this->error('添加首页模块失败');
	        }
		}
		return ZBuilder::make('form')
			->setPageTitle('添加首页模块') // 设置页面标题
	...
	public function edit($id = null)
	{
        ...
        if ($this->request->isPost()) {
            ...
            if (Db::name("icon_url")->where('id',$id)->update($data)) {
                return $this->success('修改首页模块成功');
            } else {
                return $this->error('修改首页模块失败');
            }
        }
