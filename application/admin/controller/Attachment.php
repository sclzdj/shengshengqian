...
class Attachment extends Admin
{
    ...
    private function saveFile($dir = '', $from = '', $module = '')
    {
        ...
        if($info){
            ...
            //判断是否写入远程七牛
            if(config('qiniu_open') == "1")
            {
                if( config('qiniu_accessKey') && config('qiniu_secretKey') && config('qiniu_bucket') && config('qiniu_domain'))
                {
                    include_once ROOT_PATH.DS.'lib'.DS.'qiniu'.DS.'qiniu.php';
                    $qiniu = new \Helper_qiniu(config('qiniu_accessKey'), config('qiniu_secretKey'), config('qiniu_bucket'));
                    $up = $qiniu->uploadFile($info->getSaveName(), $file_info['path']);
                    if($up !== false)
                    {
                        $file_info['driver'] = 'remote';
                        $file_info['url'] =config('qiniu_domain').'/'.$up;
...
//                 $file_path = PUBLIC_PATH. $file_info['path'];
                switch ($from) {
                    ...
                    case 'editormd':
                        ...
                        break;
                    case 'ckeditor':
                        return ck_js($callback, $file_info['path']);
                        break;
                    default:
                        return json([
                            'status' => 1,
