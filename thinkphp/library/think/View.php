...
class View
{
     ...
     * @param string      $basicTemplate     自定义基础模板
     * @return string
     * @throws Exception
     */
    public function fetch($template = '', $vars = [], $replace = [], $config = [], $renderContent = false,$basicTemplate='')
    {
        // 模板变量
        $vars = array_merge($this->data, $vars);
        //实例化自定义的基础模板
        if(!empty($basicTemplate)){
            $vars['_admin_base_layout'] = APP_PATH . 'admin/view/'.$basicTemplate.'.html';
        }
        // 页面缓存
        ob_start();
    ...
    public function getFetch($template = '', $vars = [], $replace = [], $config = [], $renderContent = false)
    {
        // 模板变量
        $vars = array_merge($this->data, $vars);
